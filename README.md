...
| **Inference detectors** | `inference_score`, `inference_likely_bool`, `inference_sem_score`, `inference_sem_likely_bool` | Heuristics for inferred answers: the classic rule (low lexical overlap + strong typed support) and a semantic variant using MiniLM QA embeddings | `--enable-inference-signal` |
...
| **QA semantic alignment** | `qr_alignment.cosine_embed`, `qr_alignment.answer_covers_question_sem`, `answer_novelty_ratio`, `qa_context_coverage_gap` | MiniLM embedding similarity between question and answer; novelty ratio compares lexical precision to semantic similarity; coverage gap highlights cases where the question is repeated but the best context similarity is low | Enabled automatically when `sentence-transformers` MiniLM model is available (record `--use-embed-alignment` for bookkeeping) |
| **Answer↔context semantic alignment** | `context_alignment.answer_context_similarity`, `context_alignment.best_context_similarity`, `semantic_gap` | Embedding similarity between the answer and each retrieved context chunk, plus a simple semantic gap proxy | Same as above (requires MiniLM embeddings) |
| **Per-sentence alignment buckets** | `per_sentence_semantic_alignment`, `per_sentence_semantic_alignment_low_frac`, `_medium_frac`, `_high_frac` | `per_sentence_semantic_alignment` = `context_alignment.best_context_similarity` (MiniLM cosine in [-1,1]) divided by the number of answer sentences; the three “_frac” fields show the share of sentences falling into low/medium/high lexical bins (<0.3, 0.3–0.6, ≥0.6) | Always on |
